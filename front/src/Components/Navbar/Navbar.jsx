...
const LaptopNavbar = () => {
  return (
    <nav className="navbarContainer fixed top-0 left-0 right-0 flex w-full p-4 bg-yellow border border-b-blue-900/50">
      <Link to="/home" className="navbarLeft flex-1 w-1/3">
        Home
      </Link>
      <Link to="/about" className="navbarCenter flex-1 w-5/12">
        About
      </Link>
      <Link to="/profile" className="navbarRight flex-1 w-2/5">
        <Avatar />
      </Link>
    </nav>
  );
...
const MobileNavbar = () => {
  return (
    <nav className="navbarContainer fixed bottom-0 left-0 right-0 flex w-full justify-center items-center p-4 text-blue bg-yellow border border-t-blue-900/50">
      ...
      <Link
        to="/profile"
        className="w-1/5 h-7 flex justify-center items-center"
      >
        <HiOutlineUser />
      </Link>
    </nav>
